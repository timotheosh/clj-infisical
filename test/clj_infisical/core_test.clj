  ...
  (testing "resolve-credentials! is never invoked when client-id/client-secret are supplied"
    (with-redefs [creds/resolve-credentials! tu/unreachable!
                  auth/login! (fn [_site-url given-creds]
                                ...
                                tu/access-token-fixture)
                  secrets/fetch-secret! (constantly {:secret-value "shh"})]
...
(deftest get-secret-raw!-returns-full-map-test
  (testing "returns the full Secret map untouched -- the 'raw' half"
    (with-redefs [creds/resolve-credentials! (constantly tu/env-credentials-fixture)
                  auth/login! (constantly tu/access-token-fixture)
                  secrets/fetch-secret! (constantly secret-fixture)]
      (is (= secret-fixture (core/get-secret-raw! base-args))))))
